...
inline void operator>>(InputStream& i_stream, PPSongChannelState& o_value) {
	...
	SEPARATOR_I
	i_stream.read(reinterpret_cast<char*>(&o_value) + offsetof(PPSongChannelState,m_songTriggerGameObject1) + sizeof(SongTriggerGameObject*), 8);
	SEPARATOR_I
	i_stream >> l_objectIndex;
	if (l_playLayer && l_objectIndex != -1) {
	...
	SEPARATOR_I
	i_stream.read(reinterpret_cast<char*>(&o_value) + offsetof(PPSongChannelState,m_songTriggerGameObject2) + sizeof(SongTriggerGameObject*), 8);
	SEPARATOR_I
}
...
inline void operator<<(OutputStream& o_stream, PPSongChannelState& i_value) {
	...
	SEPARATOR_O
	o_stream.write(reinterpret_cast<char*>(&i_value) + offsetof(PPSongChannelState,m_songTriggerGameObject1) + sizeof(SongTriggerGameObject*), 8);
	SEPARATOR_O
	l_objectIndex = -1;
	...
	o_stream << l_objectIndex;
	SEPARATOR_O
	o_stream.write(reinterpret_cast<char*>(&i_value) + offsetof(PPSongChannelState,m_songTriggerGameObject2) + sizeof(SongTriggerGameObject*), 8);
	SEPARATOR_O
}
...
void PPSongChannelState::describe() {
	...
	log::info("[PPSongChannelState - describe] pad_1: [{}]", hexStr(reinterpret_cast<unsigned char*>(this) + offsetof(PPSongChannelState,m_songTriggerGameObject1) + sizeof(SongTriggerGameObject*), 8));
	l_objectIndex = -1;
	...
	log::info("[PPSongChannelState - describe] pad_2: [{}]", hexStr(reinterpret_cast<unsigned char*>(this) + offsetof(PPSongChannelState,m_songTriggerGameObject2) + sizeof(SongTriggerGameObject*), 8));
}
#endif
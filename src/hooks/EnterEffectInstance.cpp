...
inline void operator>>(InputStream& i_stream, PPEnterEffectInstance& o_value) {
	...
	MAP_SEPARATOR_I
	i_stream.read(reinterpret_cast<char*>(&o_value) + offsetof(PPEnterEffectInstance,m_enterEffectAnimMap) + sizeof(gd::map<int,EnterEffectAnimValue>), 140);
	SEPARATOR_I
	int l_objectIndex;
	...
	SEPARATOR_I
	i_stream.read(reinterpret_cast<char*>(&o_value) + offsetof(PPEnterEffectInstance,m_gameObject) + sizeof(GameObject*), 28);
	VEC_SEPARATOR_I
	i_stream >> o_value.m_unkVecInt;
	VEC_SEPARATOR_I
	i_stream.read(reinterpret_cast<char*>(&o_value) + offsetof(PPEnterEffectInstance,m_unkVecInt) + sizeof(gd::vector<int>), 4);
	SEPARATOR_I
}
...
inline void operator<<(OutputStream& o_stream, PPEnterEffectInstance& i_value) {
	...
	MAP_SEPARATOR_O
	o_stream.write(reinterpret_cast<char*>(&i_value) + offsetof(PPEnterEffectInstance,m_enterEffectAnimMap) + sizeof(gd::map<int,EnterEffectAnimValue>), 140);
	SEPARATOR_O
	int l_objectIndex = -1;
	...
	SEPARATOR_O
	o_stream.write(reinterpret_cast<char*>(&i_value) + offsetof(PPEnterEffectInstance,m_gameObject) + sizeof(GameObject*), 28);
	VEC_SEPARATOR_O
	o_stream << i_value.m_unkVecInt;
	VEC_SEPARATOR_O
	o_stream.write(reinterpret_cast<char*>(&i_value) + offsetof(PPEnterEffectInstance,m_unkVecInt) + sizeof(gd::vector<int>), 4);
	SEPARATOR_O
}
...
void PPEnterEffectInstance::describe() {
	...
	log::info("[PPEnterEffectInstance - describe] pad_1: [{}]", hexStr(reinterpret_cast<unsigned char*>(this) + offsetof(PPEnterEffectInstance,m_enterEffectAnimMap) + sizeof(gd::map<int,EnterEffectAnimValue>), 140));
	int l_objectIndex = -1;
	if (m_gameObject) {
		PPPlayLayer* l_playLayer = static_cast<PPPlayLayer*>(PlayLayer::get());
	...
	log::info("[PPEnterEffectInstance - describe] pad_3: [{}]", hexStr(reinterpret_cast<unsigned char*>(this) + offsetof(PPEnterEffectInstance,m_unkVecInt) + sizeof(gd::vector<int>), 4));
}
#endif